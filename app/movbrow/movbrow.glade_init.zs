...
调用‘命令行加回调’、
	...
	上代码、
	-nm、、0、下代码
		赋予‘参数0’【主】以/tmp/先调用‘串替换方法’、先调用‘子串方法’、‘缓存目录’、5、-3了、/、.了。
	上代码、
	、、1、下代码
		当‘参数1’先
...
定义【主】扫描视频以下代码
	...
	循环先
		...
		如果调用‘头匹配方法’为‘目录’为-那么先
			赋予目录以调用‘子串方法’为‘目录’为1。
			如果‘目录’那么
				显示跳过‘目录’↙。
			再来。
		了。
	...
	循环先
		...
		当先
			...
			那么先
				...
				赋予名以调用‘正则格’、‘名’、([^.]*)。
				赋予名2以调用‘加反斜杠’、‘名’、R。
				赋予搜图以调用‘dir.begin’、‘目录2’、^‘名2’\..*(PNG|JPG|JPEG)$、l。
				赋予缩略图以调用‘dir.next’、‘搜图’。
				调用‘dir.end’、‘搜图’。
				如果‘缩略图’那么赋予缩略图以‘目录2’‘缩略图’否则先
					赋予缩略图以得缓存缩略图名。
					如果调用‘exist’为‘缩略图’等于0那么赋予缩略图以‘需要同名图片’。
				了。
				赋予zs以‘目录2’‘名’.zs.note。
				如果调用‘exist’、‘zs’那么显示‘zs’换行，加载‘zs’。
			了。
		了。
		如果存在“【主】结束”那么显示放弃↙，结束。
...
定义【主】播放当前视频以下代码
	...
	上代码那么先
		...
		如果存在-nm【主】那么先
			如果调用‘尾匹配方法’为‘视频’为.m3u那么赋予-nm以‘-nm’.m3u。
			调用‘建符号链接’、‘视频’、‘-nm’、1。
			赋予视频以‘-nm’。
		了。
