...
static void call4__(VteTerminal* vte,  int argc, const char* arg1, const char* arg2, void* data_in, s1___* contents_changed_s1 = NULL) {
	...
	call4__(vte_view___::scrolled_from__(vte), window__(v), s1, argc, arg1, arg2);
	if(s1 == contents_changed_s1) {
		v->outext_lock_ = false;
	}
}
...
static void cb__(VteTerminal* vte, void* data_in) {
	call4__(vte, 0, NULL, NULL, data_in, contents_changed_s1_);
}
...
vte_view___* vte_shell___::view__(const string& name, int& page_num, const string& help) {
	vte_view___* v = (vte_view___*)get_view__(name, page_num, help);
	if(!v) {
		err_wufa__(name, help.c_str());
	}
	return v;
}
...
bool vte_shell___::api__(void* shangji, void* ce, deque<string>* p, char* buf, long siz, char** addr_ret) {
	if(p->size() >= 2) {
		...
		vte_view___* v;
		if(p1 == "输入") {
			v=view__(p0,page_num,p1);if(!v)return true;
			if(err_buzu2__(p, 3))
				return true;
		...
		if(p1 == new_view_s1_->src2__())
			return api_new_view__(p0, p1, p, new_view_s1_);
	}
