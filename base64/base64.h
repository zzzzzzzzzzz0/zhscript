...
inline_ int Base64Encode(char * base64code, const char * src, int src_len = 0); 
inline_ int Base64Decode(char * buf, const char * base64code, int src_len = 0);

inline_ char GetB64Char(int index)
{
...
// 编码后的长度一般比原文多占1/3的存储空间，请保证base64code有足够的空间
inline_ int Base64Encode(char * base64code, const char * src, int src_len) 
{   
	if (src_len == 0)
		src_len = strlen(src);
...
inline_ int GetB64Index(char ch)
{
	int index = -1;
	if (ch >= 'A' && ch <= 'Z')
	{
...
inline_ int Base64Decode(char * buf, const char * base64code, int src_len) 
{   
	if (src_len == 0)
